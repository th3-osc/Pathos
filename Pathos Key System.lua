...
if PathosLoader then
	local BlurEffect = game:GetService("Lighting"):FindFirstChild("PathosBlur") do
		if BlurEffect then
			BlurEffect:Destroy()
		end
	end
...
local API = loadstring(game:HttpGet("https://sdkAPI-public.luarmor.net/library.lua"))() do
...
setupHoverEffect(GetKeyButton, Color3.fromRGB(60, 60, 65), Color3.fromRGB(80, 80, 85))
...
CloseButton.MouseLeave:Connect(function()
	CreateTween(CloseButton, {TextColor3 = Color3.fromRGB(200, 200, 200)}, 0.2):Play()
end)

PopupCloseButton.MouseEnter:Connect(function()
	CreateTween(PopupCloseButton, {TextColor3 = Color3.fromRGB(255, 100, 100)}, 0.2):Play()
end)

PopupCloseButton.MouseLeave:Connect(function()
	CreateTween(PopupCloseButton, {TextColor3 = Color3.fromRGB(200, 200, 200)}, 0.2):Play()
end)

DiscordSupportButton.MouseEnter:Connect(function()
	CreateTween(DiscordSupportButton, {TextColor3 = Color3.fromRGB(220, 50, 50)}, 0.2):Play()
end)

DiscordSupportButton.MouseLeave:Connect(function()
	CreateTween(DiscordSupportButton, {TextColor3 = Color3.fromRGB(150, 150, 150)}, 0.2):Play()
end)
...
local function showStatus(message, color, duration)
	...
	task.spawn(function()
		...
		if StatusLabel and StatusLabel.Parent then
			CreateTween(StatusLabel, {TextTransparency = 1}, 0.3).Completed:Connect(function()
				if StatusLabel and StatusLabel.Parent then
					StatusLabel.Visible = false
...
local function createNotification(message, notificationType, duration)
	...
	local notificationColors = {
		success = {bg = Color3.fromRGB(45, 45, 52), border = Color3.fromRGB(120, 20, 20), icon = "✓"},
		error = {bg = Color3.fromRGB(45, 45, 52), border = Color3.fromRGB(180, 30, 30), icon = "✗"},
		info = {bg = Color3.fromRGB(45, 45, 52), border = Color3.fromRGB(100, 100, 110), icon = "ℹ"},
		warning = {bg = Color3.fromRGB(45, 45, 52), border = Color3.fromRGB(160, 80, 20), icon = "⚠"}
	}

	local config = notificationColors[notificationType] or notificationColors.info
	...
	closeButton.MouseLeave:Connect(function()
		CreateTween(closeButton, {TextColor3 = Color3.fromRGB(180, 180, 180)}, 0.2):Play()
	end)
	...
	task.spawn(function()
		for i = 1, shakeCount do
			...
			CreateTween(MainFrame, {
				Position = UDim2.new(originalPosition.X.Scale, originalPosition.X.Offset + randomX, 
...
local function openPopup()
	...
	CreateTween(KeyProviderPopup, {Size = UDim2.new(0, 280, 0, 200)}, 0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out):Play()
end
...
local function closeUI()
	...
	local backgroundTween = CreateTween(BackgroundFrame, {BackgroundTransparency = 1}, 0.4)
...
DiscordSupportButton.MouseButton1Click:Connect(function()
	task.spawn(function()
		...
		pcall(function()
			local request = request or http_request or (syn and syn.request) do
				if request then
					request({
						Url = "http://127.0.0.1:6463/rpc?v=1",
						Method = "POST",
						Headers = { ["Content-Type"] = "application/json", ["origin"] = "https://discord.com" },
						Body = HttpService:JSONEncode({ args = { code = "pathoscc" }, cmd = "INVITE_BROWSER", nonce = "." }),
					})
					createNotification("Opening Discord support server...", "info", 3)
				else
					createNotification("Discord RPC not available", "warning", 4)
				end
			end
			if setclipboard then
